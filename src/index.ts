...
import {
	completeManualCodexLogin,
	createManualCodexLogin,
	type ManualCodexLogin,
} from "./codex-oauth.js";
import {
...
type ProviderAuthResult = {
	data?: Record<string, ProviderAuthMethod[]>;
	error?: unknown;
};
...
type ProviderOauthAuthorizeResult = {
	data?: ProviderAuthAuthorization;
	error?: unknown;
};
...
type ServerProviderClient = {
	...
	provider: {
		...
		oauth: {
			authorize(
				parameters: ProviderOauthAuthorizeParameters,
...
export const tui = async (
	inputApi: TuiPluginApi | Record<string, unknown>,
): Promise<void> => {
	...
	async function showSwitchProfileDialog(): Promise<void> {
		...
		if (savedProfiles.length === 0) {
			showToast(
				"warning",
				"No saved OpenAI profiles found. Save the current profile first.",
			);
			return;
		}
			...
			api.ui.DialogSelect({
				...
				onSelect: (option) => {
					...
					void runSafely(async () => {
						const switchedProfile = await switchActiveOpenAIProfile(
							paths,
							option.value.name,
						);
						const activeProfile = await getActiveOpenAIProfile(paths);
						const didApplyActiveProfile =
							await applyActiveOpenAIProfile(activeProfile);
						const message = didApplyActiveProfile ? "" : ` ${RESTART_MESSAGE}`;

						showToast(
							"success",
							`Switched to ${switchedProfile.name}.${message}`,
						);
					});
				},
			}),
		...
		api.ui.dialog.replace(() =>
			api.ui.DialogPrompt({
				...
				onConfirm: (profileName) => {
					api.ui.dialog.clear();
					void runSafely(async () => {
						const savedProfile = await saveActiveOpenAIProfile(
							paths,
							profileName,
						);
			...
			api.ui.DialogPrompt({
				...
				onConfirm: (nextName) => {
					...
					void runSafely(async () => {
						...
						showToast(
							"success",
							`Renamed ${profile.name} to ${renamedProfile.name}.`,
						);
					});
				},
				onCancel: () => {
	...
	async function showActiveAccount(): Promise<void> {
		if (!(await authFileExists(paths))) {
			showToast(
				"warning",
				"No opencode auth.json found. Log in to OpenAI first.",
			);
			return;
		}

		const activeProfile = await getActiveOpenAIProfile(paths);
		const savedProfileDescription = await getSavedProfileDescription(
			paths,
	...
	async function saveCurrentProfileBeforeLogin(): Promise<void> {
		...
		if (savedProfile) {
			showToast(
				"success",
				`Saved current profile as ${savedProfile.name} before login.`,
			);
		}
	}

	async function startOpenAILogin(
		method: SelectedOpenAIAuthMethod,
	): Promise<void> {
		if (method.label === OPENAI_HEADLESS_LOGIN_METHOD_LABEL) {
		...
		if (
			authorizationResult.data.method === "code" ||
		...
		showToast(
			"info",
			"Opened OpenAI login. Keep opencode open until login completes.",
		);
		void runSafely(async () => completeOpenAILogin(method.index));
	}
	...
	function showHeadlessLoginInstructions(
		methodLabel: string,
...
function isTuiPluginApi(
	api: TuiPluginApi | Record<string, unknown>,
): api is TuiCommandLayerApi {
...
async function getSavedProfileDescription(
	paths: ReturnType<typeof getOpenCodeAuthPaths>,
	accountId: string | undefined,
): Promise<string> {
	const matchingProfileNames = await getMatchingSavedProfileNames(
		paths,
		accountId,
	);
...
async function getMatchingSavedProfileNames(
	paths: ReturnType<typeof getOpenCodeAuthPaths>,
	accountId: string | undefined,
): Promise<string[]> {
	if (!accountId) {
...
async function handleServerCommand(
	...
	try {
		...
		if (action === "list") {
			...
			const profileNames =
				profiles.map((profile) => profile.name).join(", ") || "none";
...
async function startServerOpenAILogin(
	...
	const parsedMethodPreference =
		parseOpenAIAuthMethodPreference(methodPreference);
	...
	const openAIAuthMethods = authMethodsResult.data[OPENAI_PROVIDER_ID] ?? [];
	const methodIndex = findOpenAIAuthMethodIndex(
		openAIAuthMethods,
		parsedMethodPreference,
	);

	if (methodIndex === undefined) {
		const availableMethods =
			openAIAuthMethods.map((method) => method.label).join(", ") || "none";

		throw new Error(
			`OpenAI login method not found. Available: ${availableMethods}`,
		);
	}
...
function getOpenCommand(pathOrUrl: string): {
	executable: string;
	args: string[];
} {
