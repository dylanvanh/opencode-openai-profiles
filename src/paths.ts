...
export function getOpenCodeAuthPaths(environment: NodeJS.ProcessEnv = process.env): OpenCodeAuthPaths {
  const xdgDataHome = environment.XDG_DATA_HOME?.trim();
  const dataHomePath = xdgDataHome && xdgDataHome.length > 0 ? xdgDataHome : join(homedir(), ".local", "share");
